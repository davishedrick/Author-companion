...
function normalizeSession(session) {
  ...
  return {
    ...
    wordsAdded: number(session?.wordsAdded),
    wordsRemoved: number(session?.wordsRemoved),
    netWordsChanged: number(session?.netWordsChanged),
    notes: String(session?.notes || ""),
    source: String(session?.source || ""),
    documentId: String(session?.documentId || ""),
    documentUrl: String(session?.documentUrl || ""),
    extensionSessionId: String(session?.extensionSessionId || ""),
    wordCountMethod: String(session?.wordCountMethod || ""),
    startDocumentWordCount: number(session?.startDocumentWordCount),
    endDocumentWordCount: number(session?.endDocumentWordCount),
...
function normalizeIssue(issue) {
  ...
  return {
    ...
    documentId: String(issue?.documentId || ""),
    documentUrl: String(issue?.documentUrl || ""),
    extensionIssueId: String(issue?.extensionIssueId || ""),
    quoteLocator: issue?.quoteLocator && typeof issue.quoteLocator === "object"
      ? cloneValue(issue.quoteLocator)
      : null
  };
}
...
function getEditProjectStats(bundle) {
  const sessions = Array.isArray(bundle?.sessions) ? getEditSessions(bundle) : [];
  ...
  const bundle = normalizeProjectBundle({
    ...
    sessions: rows
      ...
      .map((row) => ({
        ...
        wordsAdded: number(row.session_words_added),
        wordsRemoved: number(row.session_words_removed),
        netWordsChanged: number(row.session_net_words),
        focusKey: row.session_type === "edit"
          ? normalizeEditFocusKey(row.session_pass_name, projectRow.project_edit_pass_name || projectRow.project_edit_pass_stage)
          : "",
