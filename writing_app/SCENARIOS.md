...
| PS-DEL-002 | Delete while remote refresh is in-flight | Project state | Focus the browser tab (triggering a background GET), then immediately archive and delete a project before the GET completes. | Project is removed and stays removed. The in-flight GET does not overwrite the deletion; the saved snapshot is flushed as a PUT after the GET finishes. | Automated (static) | `tests/test_app.py::test_pending_snapshot_guard_prevents_in_flight_get_from_overwriting_deletion` | In Safari: focus the tab, quickly delete a project — must not reappear. | APP-2026-06-13-deleted-project-reappears-in-safari-get-in-flight |
